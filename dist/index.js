...
function SlotMachine({
  ...
  if (children) {
    return /* @__PURE__ */ jsx("div", { className: classNames, style, children });
  }
  return /* @__PURE__ */ jsxs("div", { className: classNames, style, children: [
    /* @__PURE__ */ jsx("div", { className: "reeljs-slot-machine__reels", children: Array.from({ length: reelCount }, (_, i) => /* @__PURE__ */ jsx(
      Reel,
      {
        symbols: symbolIds,
        spinning: spinning[i],
    ...
    showStopButtons && /* @__PURE__ */ jsx("div", { className: "reeljs-slot-machine__buttons", children: Array.from({ length: reelCount }, (_, i) => /* @__PURE__ */ jsx(
      StopButton,
      {
        reelIndex: i,
...
function useSlotMachine(config) {
  const engineRef = useRef(new SpinEngine(config.spinEngine));
...
function useGameMode(config) {
  const managerRef = useRef(new GameModeManager(config));
  const manager = managerRef.current;
  const [currentMode, setCurrentMode] = useState(manager.currentMode);
  const [currentBonusType, setCurrentBonusType] = useState(manager.currentBonusType);
  const [remainingSpins, setRemainingSpins] = useState(manager.getRemainingSpins());
  const registeredRef = useRef(false);
...
function useGameCycle(config) {
  const managerRef = useRef(new GameCycleManager(config));
  const manager = managerRef.current;
  const [currentPhase, setCurrentPhase] = useState(manager.currentPhase);
  ...
  const onPhase = useCallback((phase, callback) => {
...
function useGameZone(config) {
  const managerRef = useRef(new ZoneManager(config));
  const manager = managerRef.current;
  ...
  const [indicator, setIndicator] = useState(manager.indicator);
...
function useCredit(config) {
  const managerRef = useRef(new CreditManager(config));
  const manager = managerRef.current;
...
function useNotification(config) {
  ...
  const onPreSpin = useCallback((cb) => {
    callbacksRef.current["PRE_SPIN"] = cb;
  }, []);
  const onPostSpin = useCallback((cb) => {
    callbacksRef.current["POST_SPIN"] = cb;
  }, []);
  const onNextBet = useCallback((cb) => {
    callbacksRef.current["NEXT_BET"] = cb;
  }, []);
  const onLeverOn = useCallback((cb) => {
...
function useThresholdTrigger(configs) {
  ...
  const triggerRef = useRef(new ThresholdTrigger(configs));
  const counter = counterRef.current;
  const trigger = triggerRef.current;
  const [counters, setCounters] = useState(counter.getAll());
  const [thresholds, setThresholds] = useState(trigger.getAllThresholds());
...
function useDifficulty(config) {
  const presetRef = useRef(new DifficultyPreset(config));
  const preset = presetRef.current;
  const [currentLevel, setCurrentLevel] = useState(preset.currentLevel);
  const [currentConfig, setCurrentConfig] = useState(preset.currentConfig);
...
export { AnimationController, ConfigSerializer, CreditManager, DifficultyPreset, EventEmitter, GameCycleManager, GameModeManager, InternalLottery, NotificationManager, Reel, ReelController, SlotMachine, SpinCounter, SpinEngine, StopButton, Symbol, ThresholdTrigger, ZoneManager, useCredit, useDifficulty, useEvent, useGameCycle, useGameMode, useGameZone, useNotification, useSlotMachine, useSoundEffect, useThresholdTrigger };
